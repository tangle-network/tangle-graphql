...
const TANGLE_LOCAL_WS_RPC_ENDPOINT = 'ws://127.0.0.1:9944';
const TANGLE_TOKEN_DECIMALS = 18;

const MINIMUM_BALANCE_UINT = '1';
const MINT_AMOUNT = '1000';
const POOL_ID = 1;
const APY = 12;
const CAP = formatBalance('2000', { decimals: TANGLE_TOKEN_DECIMALS });
...
async function main() {
  ...
  await batchTxes([
    api.tx.assets.create(
      tgTEST.id,
      ALICE_SUDO.address,
      formatBalance(MINIMUM_BALANCE_UINT, { decimals: tgTEST.decimals }),
    ),
    api.tx.assets.create(
      tgtTNT.id,
      ALICE_SUDO.address,
      formatBalance(MINIMUM_BALANCE_UINT, { decimals: tgtTNT.decimals }),
    ),
  ]);
  ...
  await batchTxes([
    api.tx.assets.mint(
      tgTEST.id,
      ALICE_SUDO.address,
      formatBalance(MINT_AMOUNT, { decimals: tgTEST.decimals }),
    ),
    api.tx.assets.mint(
      tgtTNT.id,
      ALICE_SUDO.address,
      formatBalance(MINT_AMOUNT, { decimals: tgtTNT.decimals }),
    ),
    api.tx.assets.mint(
      tgTEST.id,
      BOB.address,
  ...
  await api.tx.balances
    .transferKeepAlive(PALLET_ACCOUNT, formatBalance('1', { decimals }))
  ...
  await api.tx.multiAssetDelegation
    .joinOperators(
      formatBalance(MINIMUM_BALANCE_UINT, { decimals: TANGLE_TOKEN_DECIMALS }),
    )
